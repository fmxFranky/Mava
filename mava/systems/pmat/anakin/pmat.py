...
from mava.networks.pmat_network import PrioritizedMultiAgentTransformer
from mava.systems.pmat.types import ActorApply, LearnerApply, LearnerState, PMATTransition
...
def get_learner_fn(
    ...
    def _update_step(learner_state: LearnerState, _: Any) -> Tuple[LearnerState, Tuple]:
        ...
        ) -> Tuple[LearnerState, Tuple[PMATTransition, Metrics]]:
            """Step the environment."""
            params, opt_state, key, env_state, last_timestep = learner_state

            # Select action
            key, policy_key = jax.random.split(key)
            action, log_prob, value, seq_log_prob, seq = actor_action_select_fn(  # type: ignore
                params,
        ...
        def _calculate_gae(
            traj_batch: PMATTransition, last_val: chex.Array
        ) -> Tuple[chex.Array, chex.Array]:
            """Calculate the GAE."""

            def _get_advantages(gae_and_next_value: Tuple, transition: PMATTransition) -> Tuple:
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                ...
                def _loss_fn(
                    params: FrozenDict,
                    traj_batch: PMATTransition,
                    gae: chex.Array,
                    value_targets: chex.Array,
                    entropy_key: chex.PRNGKey,
                ) -> Tuple:
                    """Calculate the PMAT loss."""
                    # Rerun network
                    log_prob, value, entropy, seq_log_prob, seq_entropy = actor_apply_fn(  # type: ignore
                        params,
                        traj_batch.obs,
                        traj_batch.action,
                        traj_batch.seq,
                    ...
                    # Normalise advantage at minibatch level
                    ...
                    # Sequence loss (ranking loss for PMAT)
                    seq_ratio = jnp.exp(seq_log_prob - traj_batch.seq_log_prob)
                    # Use mean advantage for sequence-level loss
                    # Handle both reshaped (minibatch) and original gae shapes
                    if len(gae.shape) >= 2:
                        seq_advantages = gae.mean(
                            axis=-1, keepdims=True
                        )  # Mean over agent dimension
                    else:
                        seq_advantages = gae.mean(keepdims=True)  # For 1D case
                    seq_loss1 = seq_ratio * seq_advantages.squeeze()
                    seq_loss2 = (
                        jnp.clip(
                            seq_ratio,
                            1.0 - config.system.clip_eps,
                        ...
                        * seq_advantages.squeeze()
                    )
                    ranking_loss = -jnp.minimum(seq_loss1, seq_loss2).mean()
                    ...
                    total_loss = (
                        ...
                        + config.network.ranking_loss_coef * ranking_loss
                        - config.system.ent_coef * seq_entropy.mean()
                    )
                    return total_loss, (
                        actor_loss,
                        entropy,
                ...
                total_loss, (actor_loss, entropy, value_loss, ranking_loss, seq_entropy) = (
                    actor_loss_info
                )
                loss_info = {
            ...
            # Define function to safely shuffle along agent axis
            def safe_agent_shuffle(x):
                # Only shuffle along axis=1 if the array has at least 2 dimensions
                # and the second dimension matches num_agents
                if len(x.shape) >= 2 and x.shape[1] == config.system.num_agents:
                    return jnp.take(x, permutation, axis=1)
                else:
                    return x  # Return unchanged for scalar-like quantities (e.g., seq_log_prob)

            shuffled_batch = tree.map(safe_agent_shuffle, shuffled_batch)

            # Define function to safely reshape into minibatches
            def safe_reshape_minibatch(x):
                if len(x.shape) >= 2:
                    return jnp.reshape(x, (config.system.num_minibatches, -1, *x.shape[1:]))
                else:
                    # For 1D arrays, reshape to (num_minibatches, -1)
                    return jnp.reshape(x, (config.system.num_minibatches, -1))

            minibatches = tree.map(safe_reshape_minibatch, shuffled_batch)
...
def learner_setup(
    ...
    # Initialize sequence for PMAT
    init_seq = jnp.arange(config.system.num_agents)[None, :]  # (1, N)

    # Define network and optimiser.
    actor_network = PrioritizedMultiAgentTransformer(
    ...
    params = actor_network.init(actor_net_key, init_x, init_action, init_seq, jax.random.PRNGKey(0))
...
def run_experiment(_config: DictConfig) -> float:
    """Runs experiment."""
    _config.logger.system_name = "pmat"
    ...
    def eval_act_fn(
        ...
        Given that the PMAT network has a `get_actions` method we define this eval_act_fn
        accordingly.
        """

        del actor_state  # Unused since the system doesn't have memory over time.
        output_action, _, _, _, _ = actor_network.apply(  # type: ignore
            params,
...
def hydra_entry_point(cfg: DictConfig) -> float:
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}PMAT experiment completed{Style.RESET_ALL}")
    return eval_performance
