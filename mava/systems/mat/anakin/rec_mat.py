...
from mava.systems.mat.types import LearnerApply
...
# Hidden states are now imported from the network module


class RecMATLearnerState(NamedTuple):
    """State of the recurrent MAT learner."""
    ...
    env_state: Any
    timestep: TimeStep
    hstates: MATHiddenStates


# Define apply functions for recurrent MAT
RecMATActorApply = Tuple[
...
def get_init_mat_hidden_states(embed_dim: int, batch_size: int) -> MATHiddenStates:
    """Initialize hidden states for recurrent MAT."""
    return MATHiddenStates(
        encoder=jnp.zeros((batch_size, embed_dim)),
        decoder_self=jnp.zeros((batch_size, embed_dim)),
        decoder_cross=jnp.zeros((batch_size, embed_dim)),
    )


def get_learner_fn(
    env: MarlEnv,
    apply_fns: RecMATActorApply,
    update_fn: optax.TransformUpdateFn,
    config: DictConfig,
) -> LearnerFn[RecMATLearnerState]:
    ...
    def _update_step(learner_state: RecMATLearnerState, _: Any) -> Tuple[RecMATLearnerState, Tuple]:
        ...
        Args:
            learner_state (NamedTuple):
                ...
                - hstates: The hidden states.
            _ (Any): The current metrics info.
        """

        def _env_step(
            learner_state: RecMATLearnerState, _: Any
        ) -> Tuple[RecMATLearnerState, Tuple[PPOTransition, Metrics]]:
            """Step the environment."""
            params, opt_state, key, env_state, last_timestep, hstates = learner_state

            # Select action
            key, policy_key = jax.random.split(key)
            action, log_prob, value, updated_hstates = actor_action_select_fn(  # type: ignore
                params,
            ...
            updated_hstates = tree.map(
                lambda hs: jnp.where(done, jnp.zeros_like(hs), hs), updated_hstates
            )
            ...
            learner_state = RecMATLearnerState(
                params, opt_state, key, env_state, timestep, updated_hstates
            )
        ...
        # Calculate advantage
        params, opt_state, key, env_state, last_timestep, updated_hstates = learner_state
        ...
        def _update_epoch(update_state: Tuple, _: Any) -> Tuple:
            ...
            params, opt_state, traj_batch, advantages, targets, key, prev_hstates = update_state
            ...
            shuffled_batch = tree.map(lambda x: jnp.take(x, batch_perm, axis=0), batch)
            ...
            mb_traj, mb_adv, mb_tgt = tree.map(
                lambda x: jnp.reshape(x, (config.system.num_minibatches, -1, *x.shape[1:])),
                shuffled_batch,
            )

            # Define minibatch update
            def _update_minibatch(train_state: Tuple, batch_info: Tuple) -> Tuple:
                """Update the network for a single minibatch."""
                params, opt_state, key, prev_hstates = train_state
                traj_batch_i, adv_i, tgt_i = batch_info

                def _loss_fn(params, traj_batch, gae, value_targets, entropy_key):
                    # 从 Observation 对象中获取 minibatch 大小
                    batch_size_local = traj_batch.obs.agents_view.shape[0]
                    # 计算 env 数量和重复因子，用于 tile 隐藏状态到 minibatch 大小
                    num_envs_local = prev_hstates.encoder.shape[0]
                    repeat_factor = batch_size_local // num_envs_local
                    # 将隐藏状态按时间步重复，以匹配 minibatch 大小
                    bhstates = tree.map(
                        lambda hs: jnp.tile(hs, (repeat_factor, 1)),
                        prev_hstates,
                    )
                    ...
                    value_loss = 0.5 * jnp.mean(
                        jnp.maximum(
                            jnp.square(value - value_targets),
                            jnp.square(value_pred_clipped - value_targets),
                        )
                    )
                    total_loss = (
                        actor_loss
                        - config.system.ent_coef * entropy_mean
                        + config.system.vf_coef * value_loss
                    )
                    return total_loss, (actor_loss, entropy_mean, value_loss)

                key, entropy_key = jax.random.split(key)
                actor_grad_fn = jax.value_and_grad(_loss_fn, has_aux=True)
                (loss, (actor_loss, entropy_mean, value_loss)), actor_grads = actor_grad_fn(
                    params, traj_batch_i, adv_i, tgt_i, entropy_key
                )
                actor_grads, (actor_loss, entropy_mean, value_loss) = jax.lax.pmean(
                    (actor_grads, (actor_loss, entropy_mean, value_loss)), axis_name="batch"
                )
                ...
                return (new_params, new_opt_state, key, prev_hstates), loss_info

            # Run minibatch updates
            (params, opt_state, entropy_key, _), loss_info = jax.lax.scan(
                _update_minibatch,
                (params, opt_state, key, prev_hstates),
        ...
        update_state = params, opt_state, traj_batch, advantages, targets, key, prev_hstates
        ...
        params, opt_state, traj_batch, advantages, targets, key, prev_hstates = update_state
        learner_state = RecMATLearnerState(
            params, opt_state, key, env_state, last_timestep, updated_hstates
        )

        return learner_state, (episode_metrics, loss_info)

    def learner_fn(learner_state: RecMATLearnerState) -> ExperimentOutput[RecMATLearnerState]:
        """Learner function.
        ...
        Args:
            learner_state (NamedTuple):
                ...
                - key: The random number generator state.
                - env_state: The environment state.
                - timsteps: The initial timestep in the initial trajectory.
                - hstates: The hidden states.
        """
...
def learner_setup(
...
) -> Tuple[LearnerFn[RecMATLearnerState], Any, RecMATLearnerState]:
    ...
    actor_network = RecurrentMultiAgentTransformer(
    ...
    init_hstates = get_init_mat_hidden_states(config.network.embed_dim, 1)
    params = actor_network.init(
        actor_net_key, init_x, init_action, init_hstates, jax.random.PRNGKey(0)
    )
    ...
    init_hstates = get_init_mat_hidden_states(config.network.embed_dim, config.arch.num_envs)
    ...
    if config.logger.checkpointing.load_model:
        ...
        restored_params, restored_hstates = loaded_checkpoint.restore_params(
            input_params=params, restore_hstates=True, THiddenState=MATHiddenStates
        )
    ...
    init_learner_state = RecMATLearnerState(
        params, opt_state, step_keys, env_states, timesteps, init_hstates
    )
...
def run_experiment(_config: DictConfig) -> float:
    """Runs experiment."""
    _config.logger.system_name = "rec_mat"
    ...
    # Setup evaluator.
    def make_rec_mat_act_fn(actor_apply_fn: Any) -> EvalActFn:
    ...
    eval_act_fn = make_rec_mat_act_fn(apply_fns[0])
    ...
    eval_hs = get_init_mat_hidden_states(config.network.embed_dim, eval_batch_size)
    ...
    if config.arch.absolute_metric:
        ...
        abs_hs = get_init_mat_hidden_states(config.network.embed_dim, eval_batch_size)
...
def hydra_entry_point(cfg: DictConfig) -> float:
    ...
    print(f"{Fore.CYAN}{Style.BRIGHT}Rec MAT experiment completed{Style.RESET_ALL}")
    return eval_performance
